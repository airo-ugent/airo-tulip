...
class TulipServer:
    ...
    def _request_loop(self):
        ...
        while not self._should_stop.is_set():
            ...
            # The REP socket requires that every recv is followed by a send, so we must always reply,
            # even when handling the request fails. Otherwise the socket wedges and the server can no
            # longer process any request.
            try:
                response = self._handle_request(request)
            except Exception as e:
                logger.exception("Unhandled error while handling client request.")
                response = ErrorResponse("Internal server error", str(e))
            # Send response.
    ...
    def _handle_request(self, request: RequestMessage) -> ResponseMessage:
        ...
        handler = self._request_handlers.get(request_class_name)
        if handler is None:
            logger.error(f"Received unknown request type: {request_class_name}.")
            return ErrorResponse("Unknown request type", request_class_name)
