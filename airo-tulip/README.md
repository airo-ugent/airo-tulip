...
The `airo-tulip` package is a Python port of the Kelo Tulip software with some additionaly functionality.
...
Later on, we'll add more types of controllers to also support eg. torque control and compliant control. These new controller classes will be added to the `airo_tulip.controllers` module.
...
The acceleration and decelleration of each drive is limited to 0.5 m/s^2 linear and 0.8 m/s^2 angular by attributes of the `PlatformLimits` dataclass that is used in the `VelocityPlatformController`.
...
The contents of these buffers is respresented by the `TxPDO1` and `RxPDO1` structs specified in `ethercat.py`.
...
Drives continuosly need the receive EtherCAT telegrams in orde for them to stay enabled.
...
This limitation poses special requirements on the implementation of the driver code, as it is run in a seperate thread to ensure that it is called without interruption.
